...
try {
    ...
    error_log("[MetroportMetroTVModule] " . $e->getMessage());
}

usort($result['data'], function($a, $b) {
    // First packages with group = 1
    if ($a['pkg_group'] == 1 && $b['pkg_group'] != 1) return -1;
    if ($a['pkg_group'] != 1 && $b['pkg_group'] == 1) return  1;
    return $a['pkg_group'] <=> $b['pkg_group'];
});
$SMARTY->assign('metroportMetroTVResult', $result);
$SMARTY->display('metroportmetrotvservicelist.html');
