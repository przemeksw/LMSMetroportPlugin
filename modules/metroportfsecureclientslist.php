...
try {
    ...
    $result = ApiInit::Success($clients, 'Lista serwisów F-SECURE pobrana pomyślnie.');

} catch (\Exception $e) {
    $result = ApiInit::Error('Błąd pobierania listy listy serwisów F-SECURE: ' . $e->getMessage());
...
{
    foreach ($result['data']['data'] as &$customer) {
...
$SMARTY->display('metroportfsecureclientslist.html');
